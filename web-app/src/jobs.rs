...
use orm::{
    input_files::{InputJob, InputJobBuilder},
    jobs::{FairShare, Job, JobState, MapFmt, ModFmt, VmrFmt},
    utils, MySqlConn,
};
...
pub(crate) async fn make_queue_jobs_list(
    conn: &mut MySqlConn,
    config: &orm::config::Config,
) -> anyhow::Result<Vec<DisplayJob>> {
    let sub_queue = config
        .get_queue(&config.execution.submitted_job_queue)
        .unwrap_or_default();
    let jobs = Job::get_jobs_in_queue(
        conn,
        &config.execution.submitted_job_queue,
        Some(&[JobState::Pending, JobState::Running, JobState::Complete]),
    )
    .await?;
    let jobs = sub_queue
        .fair_share_policy
        .order_jobs_for_display(conn, &config.execution.submitted_job_queue, jobs)
        .await?;
    // TODO: Have display job incorporate the extra information in the hash map
    let jobs = jobs
        .into_iter()
        .map(|j| DisplayJob::from(j.0))
        .collect_vec();
    Ok(jobs)
}
...
async fn get_user_jobs(
    ...
    config: &orm::config::Config,
    user: &User,
    cleaned_and_errored_submitted_after: chrono::NaiveDateTime,
) -> anyhow::Result<UserJobs> {
    ...
    Ok(UserJobs {
        emails: all_emails,
        submitted_after: cleaned_and_errored_submitted_after,
        ready_for_download: ready_jobs,
        running,
        pending,
        errored,
        cleaned,
...
impl SubmitJobForm {
    fn to_input_job(self, user_email: String) -> Result<InputJob, Vec<String>> {
        ...
        loop {
            ...
            let this_lat = self
                .locations
                .get(&format!("site-lat-{loc_ind}"))
                .map(|s| {
                    if let Ok(v) = s.parse::<f32>() {
                        Some(v)
                    } else {
                        errors.push(format!(
                            "latitude for site {this_sid} could not be parsed as a number"
                        ));
                        None
                    }
                })
