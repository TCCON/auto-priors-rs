...
impl<T: Queueable> JobManager<T> {
    ...
    pub async fn message_loop(&mut self) {
        loop {
            ...
            if let Some(m) = msg {
                ...
                let res = match m {
                    JobMessage::StartJobs => self
                        .start_jobs_entry_point()
                        .await
                        .context("Error occurred while starting jobs"),
                    JobMessage::RegenLut => self
                        .schedule_lut_regen()
                        .await
                    ...
                    JobMessage::CleanUpJobs => self
                        ...
                        .await
                        .context("Error occurred while cleaning up jobs"),
                    JobMessage::SendStatusReports => self
                        .scan_for_status_requests()
                        .await
                        .context("Error occurred while handling status requests"),
        ...
        for key in ginput_keys {
            ...
            if !lut_queue.add(lut_job).await {
                self.error_handler.report_error(
                    anyhow::anyhow!("Failed to add job to regenerate LUTs for ginput key '{key}'")
                        .as_ref(),
                )
            }
        }
    ...
    async fn clean_up_expired_jobs(&self) -> anyhow::Result<()> {
        ...
        Job::clean_up_expired_jobs(&mut conn, false)
            .await
    ...
    pub async fn reset_running_jobs(&self) -> anyhow::Result<()> {
        ...
        let running_jobs = Job::get_jobs_in_state(&mut trans, JobState::Running)
            .await
    ...
    async fn scan_for_job_submissions(&mut self) -> anyhow::Result<()> {
        let input_glob_pattern = {
            let c = self.shared_config.read().await;
        ...
        let config = &self.shared_config.read().await;
        orm::input_files::add_jobs_from_input_files(
            &mut self.pool.get_connection().await?.detach(),
            &config,
            &input_files,
            &mut self.input_file_mover,
        )
        .await?;
    ...
    async fn scan_for_status_requests(&mut self) -> anyhow::Result<()> {
        ...
        let mut conn = self.pool.get_connection().await.context(
            "Error occurred while trying to get a database connection to send status reports",
        )?;

        if let Err(errors) = orm::input_files::send_status_reports(
            &mut conn,
            &config,
            request_files,
            &mut self.input_file_mover,
    ...
    async fn add_pending_jobs_to_queues(&mut self) -> anyhow::Result<()> {
        ...
        for name in queue_names {
            ...
            let mut conn = self.pool.get_connection().await?;
            {
                ...
                this_queue
                    .clean_up_finished(&mut conn, &self.error_handler, &tmp_config)
                    .await;
            }
            ...
            info!(
                "Queue '{name}' has {} of {n_total} allowed jobs allotted, {n_running} are active",
                n_total - n_to_add
            );
            while n_to_add > 0 {
                let next_job = Job::claim_next_job_in_queue(
                    &mut conn,
                    &name,
                    &queue_options.fair_share_policy,
                )
                .await
                .with_context(|| {
                    format!("Error occurred while trying to claim the next job in queue '{name}'")
                })?;
                ...
                let runner = T::new_from_job(next_job, &*self.shared_config.read().await);
        ...
        for queue_name in pending_queues.iter() {
            if self.can_queue_start_jobs(queue_name).await {
                ...
                if let Some(queue) = self.job_queues.get_mut(queue_name) {
                    queue
                        .start(self.pool.clone(), &config, &self.error_handler)
                        .await;
                } else {
    ...
    async fn get_all_queue_names(&mut self) -> anyhow::Result<Vec<String>> {
        let mut queue_names =
            Job::get_queues_with_pending_jobs(&mut self.pool.get_connection().await?.detach())
                .await
                .with_context(|| {
                    "Error occurred while trying to retrieve the list of queues with pending jobs"
                })?;
    ...
    async fn update_queue_max_jobs(&mut self) {
        ...
        for (name, queue) in self.job_queues.iter_mut() {
            ...
            if queue.max_num_items != max_njobs {
                info!(
                    "Updating maximum number of jobs in queue '{name}' from {} to {}",
    ...
    pub async fn wait_for_jobs_to_finish(&mut self) {
        ...
        loop {
            ...
            for (name, queue) in self.job_queues.iter_mut() {
                queue
                    .clean_up_finished(&mut conn, &self.error_handler, &config)
                    .await;
    ...
    async fn stop_and_reset_jobs(&mut self) {
        ...
        for (name, queue) in self.job_queues.iter_mut() {
            queue
                .cancel_running_jobs(&mut conn, &self.error_handler)
                .await;
...
impl<T: Queueable> Queue<T> {
    ...
    pub async fn clean_up_finished(
        &mut self,
        conn: &mut MySqlPC,
        error_handler: &ErrorHandler,
        config: &Config,
...
impl ServiceJobRunner {
    async fn is_ginput_job_done(
        ...
        match inner_res {
            ...
            Err(e) => {
                ...
                job.set_errored(conn, &e, Some(email_config))
                    .await
                    .unwrap_or_else(|e| {
                        error!(
                            "Failed to set state for job {} to 'errored' because: {e}",
                            job.job_id
                        )
                    });
    ...
    async fn is_lut_job_done(
        ...
        let inner_res = match task.await {
            Ok(r) => r,
            Err(e) => {
                anyhow::bail!("Panic occurred regenerating LUTs for ginput '{ginput_key}': {e:?}")
            }
        };
        ...
        match inner_res {
            Ok(_) => Ok(true),
            Err(e) => {
                anyhow::bail!("Error occurred regenerating LUTs for ginput '{ginput_key}': {e:?}")
            }
        }
    }
    ...
    async fn cancel_ginput_job(
        ...
        if let Some(task) = join_handle {
            ...
            match task.await {
                Ok(_) => info!(
                    "Job {} was already complete when tried to cancel",
                    job.job_id
                ),
                Err(e) if e.is_cancelled() => (),
                Err(e) => {
                    anyhow::bail!(
                        "Job #{} had encountered an error before being cancelled: {e:?}",
                        job.job_id
                    );
                }
            }
    ...
    fn describe(&self) -> String {
        match self {
            Self::GinputJob {
                ...
                join_handle: _,
                force_cleanup: _,
            } => format!("ServiceJobRunner(job = {})", job.job_id),
            Self::LutRegenJob {
                ginput_key,
                join_handle: _,
                force_cleanup: _,
    ...
    async fn start(&mut self, pool: PoolWrapper, config: &Config) -> anyhow::Result<()> {
        match self {
            Self::GinputJob {
                ...
                join_handle,
                force_cleanup: _,
            } => Self::start_ginput_job(pool, job.clone(), config.clone(), join_handle).await,
            Self::LutRegenJob {
                ginput_key,
                join_handle,
                force_cleanup: _,
...
mod tests {
    ...
    async fn make_dummy_job_manager(
        ...
        config.execution.queues.insert(
            ...
            orm::config::JobQueueOptions {
                max_num_procs: TEST_QUEUE_MAX_NUM_ITEMS,
                ..Default::default()
            },
        );
    ...
    async fn test_lut_with_running_jobs() {
        ...
        let n_running = manager
            ...
            .unwrap()
            .num_jobs_running();
        let n_lut_running = manager.job_queues.get("LUT").unwrap().num_jobs_running();

        assert_eq!(
            n_running, 2,
            "Did not have the correct number of standard jobs running after adding the LUT job"
        );
        assert_eq!(
            n_lut_running, 0,
            "The LUT job started when it should not have"
        );
    }
    ...
    async fn test_lut_blocks_std_jobs() {
        ...
        assert_eq!(
            n_running_before, 1,
            "Did not have the correct number of standard jobs running before adding the LUT job"
        );
        ...
        let n_running_after = manager
            .job_queues
            .get(TEST_QUEUE_NAME)
            .unwrap()
            .num_jobs_running();
        let n_in_queue = manager
            .job_queues
            .get(TEST_QUEUE_NAME)
            .unwrap()
            .num_jobs_left();
        let n_lut_running = manager.job_queues.get("LUT").unwrap().num_jobs_running();
        let n_lut_in_queue = manager.job_queues.get("LUT").unwrap().num_jobs_left();

        assert_eq!(
            n_in_queue, 2,
            "Did not have the right number of standard jobs after adding the LUT job"
        );
        assert_eq!(
            n_running_after, 1,
            "The second standard job started even though there was an LUT job pending"
        );
    ...
    async fn test_jobs_unblock() {
        ...
        assert_eq!(
            n_in_queue, 1,
            "Did not clean up finished first job as expected"
        );
        ...
        assert_eq!(
            n_running, 1,
            "Did not start the second standard job after the LUT job finished"
        );
    }
}
