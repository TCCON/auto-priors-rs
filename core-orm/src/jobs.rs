...
use serde_json;
use sqlx::{self, Acquire, FromRow, Type};
use tabled::Tabled;
...
impl TryFrom<Job> for QJob {
    ...
    fn try_from(j: Job) -> Result<Self, Self::Error> {
        let save_dir = j
            ...
            .to_str()
            .ok_or(anyhow::anyhow!(
                "Failed to convert save_dir to a UTF string"
            ))?
            .to_owned();
        ...
        let output_file = if let Some(o) = j.output_file {
            Some(
                o.to_str()
                    .ok_or(anyhow::anyhow!(
                        "Failed to convert output_file to UTF string"
                    ))?
                    .to_owned(),
            )
        } else {
...
impl<'j> Display for VerboseDisplayJob<'j> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "  Priority {} in queue {}",
            self.job.priority, self.job.queue
        )?;
        writeln!(
            f,
            "  Dates: {} to {}",
            self.job.start_date, self.job.end_date
        )?;
        ...
        if let Some(out) = &self.job.output_file {
            ...
            writeln!(
                f,
                "  Save under {} as tarball {}",
                self.job.root_save_dir.display(),
                self.job.save_tarball
            )?;
        }
        writeln!(
            f,
            "  Formats: mod = {}, vmr = {}, map = {}",
            self.job.mod_fmt, self.job.vmr_fmt, self.job.map_fmt
        )?;
        writeln!(
            f,
            "  Processing = {}",
...
impl Display for Job {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let dates = format!(
            "{}-{}",
            self.start_date.format("%Y-%m-%d 00:00"),
            self.end_date.format("%Y-%m-%d 00:00")
        );
...
impl Job {
    ...
    pub fn to_long_string(&self) -> String {
        ...
        format!(
            "AutoModMaker Job {job_id} ({dates}, lons = {lons}, lats = {lats}, sites = {sites})"
        )
    }
    ...
    pub fn mod_output_dir(&self, site_id: &str, config: &Config) -> anyhow::Result<Vec<PathBuf>> {
        self.run_or_output_subdir(site_id, config, false, "vertical")
            .with_context(|| {
                format!(
                    "Error occurred while trying to infer .mod output sub-directory for job #{}",
                    self.job_id
                )
            })
    }
    ...
    pub fn vmr_output_dir(&self, site_id: &str, config: &Config) -> anyhow::Result<Vec<PathBuf>> {
        self.run_or_output_subdir(site_id, config, false, "vmrs-vertical")
            .with_context(|| {
                format!(
                    "Error occurred while trying to infer .vmr output sub-directory for job #{}",
                    self.job_id
                )
            })
    }
    ...
    fn run_or_output_subdir(
        ...
        let base_dir = if run_dir {
            ...
            if p.is_file() {
                anyhow::bail!(
                    "Output is a tar file for job {}, cannot get a subdirectory of it",
                    self.job_id
                );
            } else {
                p.to_path_buf()
    ...
    pub async fn get_jobs_list(
        ...
        let qjobs = if pending_and_running_only {
            ...
            sqlx::query_as!(QJob, "SELECT * FROM Jobs")
                .fetch_all(conn)
                .await?
        };

        let jobs: Result<Vec<_>, _> = qjobs.into_iter().map(|qjob| Job::try_from(qjob)).collect();
    ...
    pub async fn get_jobs_in_queue(
        ...
        let qjobs: Vec<QJob> = if let Some(states) = states {
            ...
            let mut qb =
                sqlx::QueryBuilder::<sqlx::MySql>::new("SELECT * FROM Jobs WHERE state IN ");
            qb.push_tuples(states, |mut b, s| {
                b.push_bind(*s);
            });
            qb.push(" AND queue = ");
            qb.push_bind(queue);
            ...
            sqlx::query_as!(QJob, "SELECT * FROM Jobs WHERE queue = ?", queue)
                .fetch_all(conn)
                .await?
        };

        let jobs: Vec<Job> = qjobs
            .into_iter()
    ...
    pub async fn get_job_with_id(conn: &mut MySqlConn, id: i32) -> JobResult<Job> {
        let result = sqlx::query_as!(QJob, "SELECT * FROM Jobs WHERE job_id = ?", id)
            .fetch_one(conn)
            .await?;
    ...
    pub async fn get_jobs_in_state(
        conn: &mut MySqlConn,
        state: JobState,
    ) -> anyhow::Result<Vec<Job>> {
        let jobs: Result<Vec<Job>, _> =
            sqlx::query_as!(QJob, "SELECT * FROM Jobs WHERE state = ?", state)
                .fetch_all(conn)
                .await?
                .into_iter()
                .map(|q| q.try_into())
    ...
    pub async fn summarize_active_jobs_by_submitter(
        conn: &mut MySqlConn,
    ...
    pub async fn summarize_jobs_completed_between(
        conn: &mut MySqlConn,
        start_date: NaiveDate,
        end_date: Option<NaiveDate>,
    ) -> anyhow::Result<(JobSummary, JobSummary)> {
    ...
    pub async fn get_next_job_in_queue<F: FairShare>(
        conn: &mut MySqlConn,
        queue: &str,
        fair_share: &F,
    ...
    pub async fn claim_next_job_in_queue_with_opts<F: FairShare>(
        conn: &mut MySqlConn,
        queue: &str,
        fair_share: &F,
        delay: f32,
        ...
        Err(JobError::Other(
            "Failed to claim next job due to repeated deadlocks".to_owned(),
        ))
    }
    ...
    /// `ntries = 5`.
    pub async fn claim_next_job_in_queue<F: FairShare>(
        conn: &mut MySqlConn,
        queue: &str,
        fair_share: &F,
    ) -> JobResult<Option<Job>> {
    ...
    pub async fn get_distinct_submitter_emails(conn: &mut MySqlConn) -> JobResult<Vec<String>> {
        let emails = sqlx::query!("SELECT DISTINCT(email) FROM Jobs WHERE email IS NOT NULL",)
            .fetch_all(conn)
            .await?
            .into_iter()
    ...
    pub fn expand_site_lat_lon(
        ...
        if lat_given != lon_given {
            anyhow::bail!(
                "lat and lon must both be given or not, cannot have one given and not the other"
            )
        }
    ...
    pub async fn lats_lons_sids_filled_for_date(
        &self,
        conn: &mut MySqlConn,
        date: NaiveDate,
    ...
    pub async fn add_job_from_args(
        ...
        if site_id.len() != lat.len() || site_id.len() != lon.len() {
            return Err(JobAddError::DifferentNumSidLatLon {
                n_sid: site_id.len(),
                n_lat: lat.len(),
                n_lon: lon.len(),
            });
    ...
    pub async fn set_state_by_id(
        ...
        let nrows = sqlx::query!("UPDATE Jobs SET state = ? WHERE job_id = ?", state, job_id)
            .execute(conn)
            .await?
            .rows_affected();
    ...
    pub async fn set_completed_by_id(
        conn: &mut MySqlConn,
        job_id: i32,
        output_path: &Path,
        complete_time: Option<NaiveDateTime>,
    ) -> anyhow::Result<(u64, NaiveDateTime)> {
        ...
        let nrows = sqlx::query!(
            ...
            output_path.to_str().ok_or(anyhow::anyhow!(
                "Could not convert output_file to UTF string"
            ))?,
            complete_time,
            job_id
        )
    ...
    pub async fn set_completed(
        ...
        let (n, t) =
            Self::set_completed_by_id(conn, self.job_id, output_path, complete_time).await?;
    ...
    pub async fn set_errored<E: Debug>(
        &mut self,
        conn: &mut MySqlConn,
        err: &E,
    ...
    ) -> Result<(), JobPriorityError> {
        let mut job = Job::get_job_with_id(conn, job_id).await.with_context(|| {
            format!("Error occurred while trying to retrieve job #{job_id} to set its priority")
        })?;
    ...
    pub async fn set_delete_time_by_id(
        job_id: i32,
        conn: &mut MySqlConn,
    ...
    pub fn delete_output_and_run_dir(&self) -> anyhow::Result<()> {
        ...
        if run_dir.exists() {
            debug!(
                "Cleaning up run directory {} for job {}",
                run_dir.display(),
                self.job_id
            );
            ...
            debug!(
                "Not cleaning up run directory {} for job {}, does not exist",
                run_dir.display(),
                self.job_id
            );
        }
...
pub trait FairShare {
    ...
    async fn order_jobs_for_display(
        ...
        queue: &str,
        mut jobs: Vec<Job>,
    ) -> JobResult<Vec<(Job, HashMap<&'static str, String>)>>;
}
...
impl FairShare for PrioritySubmitFS {
    ...
    async fn order_jobs_for_display(
        ...
        let out: Vec<(Job, HashMap<&'static str, String>)> =
            jobs.into_iter().map(|j| (j, HashMap::new())).collect();
        Ok(out)
    }
}
...
impl FairShare for PsuedoRoundRobinFS {
    ...
    async fn order_jobs_for_display(
        ...
        let penalites: HashMap<String, i64, RandomState> =
            HashMap::from_iter(penalties.into_iter().filter_map(|p| {
        ...
        let out: Vec<(Job, HashMap<&'static str, String>)> = jobs
            .into_iter()
            .map(|j| {
                let fs_pen = if let Some(ref e) = j.email {
                ...
                let fs_pen_str = match j.state {
                    ...
                    | JobState::Errored
                    | JobState::Complete
                    | JobState::Running => "n/a".to_string(),
                    JobState::Pending => fs_pen.to_string(),
                };

                let extra_fields = HashMap::from_iter([("FS adjustment", fs_pen_str)]);
...
pub async fn cleanup_cancelled_ginput_job(conn: &mut MySqlConn, job: &mut Job) -> JobResult<()> {
    std::fs::remove_dir_all(&job.run_dir(false)).unwrap_or_else(|e| {
        warn!(
            "Error occurred while trying to remove run directory ({}) for job {}. Error was: {e}",
            job.run_dir(false).display(),
            job.job_id
        )
    });
...
trait InnerGinputRunner: Display + Send + Sync {
    async fn run_gen_priors_for_date(
        &self,
        ginput_args: GinputAutomationArgs,
...
impl InnerGinputRunner for ShellGinputRunner {
    async fn run_gen_priors_for_date(
        ...
        let log_file = std::fs::File::create(ginput_args.save_path.join(format!(
            "ginput_job_{}_{}.out",
            ginput_args.job_id, ginput_args.start_date
        )))
    ...
    async fn run_lut_regen(&self) -> JobResult<()> {
        ...
        let log_file = std::fs::File::create(lut_log_dir.join("automation_lut_regen.err"))
            .map_err(|e| JobError::RunDirectoryError(e))?;
        let log_stderr = Stdio::from(log_file);
...
pub fn start_priors_gen_job(pool: PoolWrapper, job: Job, config: Config) -> GinputHandle {
    tokio::spawn(async move { run_priors_gen_job(pool, job, config).await })
}
...
async fn run_priors_gen_job(pool: PoolWrapper, mut job: Job, config: Config) -> anyhow::Result<()> {
    ...
    // issue.
    info!(
        "Beginning job {} for dates {} to {}",
        job.job_id, job.start_date, job.end_date
    );
    let date_iter = crate::utils::DateIterator::new(vec![(job.start_date, job.end_date)]);
    ...
    for date in date_iter {
        ...
        let res = res.with_context(|| {
            format!(
                "Error occurred setting up arguments to run date {date} in job {}",
                job.job_id
            )
        });
        let ginput_args = res?;
        let runner = get_runner_for_date(date, &job, &config).with_context(|| {
            format!(
                "Error occurred setting up inner runner for date {date} in job {}",
                job.job_id
            )
        })?;
        info!("Running {date} for job {} using {runner}", job.job_id);
        runner
            .run_gen_priors_for_date(ginput_args, config.get_sim_delay())
            .await
            .with_context(|| {
                format!(
                    "Error occurred while running ginput for date {date} in job {}",
                    job.job_id
                )
            })?;
    }
    ...
    let (make_tarball, output_path) = match job.save_tarball {
        ...
        TarChoice::Yes => (true, make_std_tar_file_name(&mut conn, &job).await?),
        TarChoice::Egi => (true, make_egi_tar_file_name(&mut conn, &job).await?),
    };
    ...
    if !make_tarball {
        ...
        std::fs::rename(job.run_dir(false), &output_path).with_context(|| {
            format!(
                "Failed to move run directory for job {} to its download location",
                job.job_id
            )
        })?;
    } else {
        ...
        let tgz_file = std::fs::File::create(&output_path).with_context(|| {
            format!(
                "Error occurred trying to create the initial .tgz file for job {}",
                job.job_id
            )
        })?;
        ...
        for entry in std::fs::read_dir(&job_dir).with_context(|| {
            format!(
                "Error occurred while selecting output files for tarball in job {}",
                job.job_id
            )
        })? {
            let entry = entry.with_context(|| {
                format!(
                    "Error getting directory entry while making tarball for job {}",
                    job.job_id
                )
            })?;
            let src_path = entry.path();
            if src_path.is_dir() {
                let dest_path = src_path.file_name().with_context(|| {
                    format!(
                        "Error getting source directory file name from {} for job {}",
                        src_path.display(),
                        job.job_id
                    )
                })?;
                ...
                archive
                    .append_dir_all(dest_path, &src_path)
                    .with_context(|| {
                        format!(
                            "Error adding directory {} to tarball for job {}",
                            src_path.display(),
                            job.job_id
                        )
                    })?;
        ...
        let encoder = archive.into_inner().with_context(|| {
            format!(
                "Error occurred while trying to finalize tar archive for job {}",
                job.job_id
            )
        })?;
        ...
        encoder.finish().with_context(|| {
            format!(
                "Error occurred while trying to finalize the gzip compression for job {}",
                job.job_id
            )
        })?;

        std::fs::remove_dir_all(&job_dir).unwrap_or_else(|_| {
            warn!(
                "Failed to remove output directory for job {} after creating the tarball",
                job.job_id
            )
        });
    }
...
fn get_runner_for_ginput(ginput: &crate::config::GinputConfig) -> Box<dyn InnerGinputRunner> {
    match ginput {
        crate::config::GinputConfig::Script { entry_point_path } => Box::new(ShellGinputRunner {
            run_ginput_path: entry_point_path.to_owned(),
        }),
    }
}
...
async fn setup_ginput_args_for_date(
    ...
    let (site_ids, lats, lons) = job
        .lats_lons_sids_filled_for_date(conn, date)
        .await
...
async fn make_std_tar_file_name(conn: &mut MySqlConn, job: &Job) -> JobResult<PathBuf> {
    let (site_ids, site_lats, site_lons) = job
        .lats_lons_sids_filled_for_date(conn, job.start_date)
        .await
        .map_err(|e| JobError::InvalidSiteLocation(e))?;
    ...
    let locstr = if nsite == 1 && nlon == 1 && nlat == 1 {
        format!(
            "{}_{}_{}",
            site_ids[0],
            utils::format_lat_str(site_lats[0], 2),
            utils::format_lon_str(site_lons[0], 2)
        )
...
fn send_job_completion_email(job: &Job, config: &Config) -> anyhow::Result<()> {
    let email = if let Some(e) = &job.email {
        ...
        info!(
            "No email for job {}, not sending completion email",
            job.job_id
        );
        return Ok(());
    };
    ...
    let download_command =
        format!("wget --user=anonymous --password=your@email.address.com -r -nH {ftp_url}");
    let subject = format!("{job} succeeded");
    let mut body = format!(
        "{} succeeded. You may retrieve it with:\n\n {download_command}\n\n",
        job.to_long_string()
    );
